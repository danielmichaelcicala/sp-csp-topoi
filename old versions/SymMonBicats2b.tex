...
\todo{talk about other structures already found}
...
\todo{example of such categories here.}
...
\todo{cite Erbele, Baez, Fong, Sobocinski, anywhere LinRel comes up} 
	As in Fong's thesis 
\todo{cite here}
...
\todo{examples here: see Rebro's paper using spans of collared smooth manifolds}



When studying intercategories in \cite{GranPare_Intercats}, Grandis and Par\'{e} looked at spans of cospans and found a lax interchange law. While restricting attention to a topoi $\cat{T}$ in \cite{Cic}, it was showed that there are bicategories $\bimonspcsp{T}$ with $\cat{T}$-objects, $\cat{T}$-cospans, and isomorphism classes of monic $\cat{T}$-spans of cospans depicted as
\[
\begin{tikzpicture}
	\node (A) at (0,0) {$\bullet$};
	\node (B) at (1,1) {$\bullet$};
...
\end{tikzpicture}
\]
There is a similar bicategory $\biepiccspsp{T}$ as well with $\cat{T}$-objects, $\cat{T}$-spans, and isomorphism classes of epic $\cat{T}$-cospans of spans 
\[
\begin{tikzpicture}
	\node (A) at (0,0) {$\bullet$};
	\node (B) at (1,1) {$\bullet$};
...
\todo{Place vector space dual here for prepping}
...
\todo{The following is a proposition 4.2.3 in Lurie, but we'll take it as def. Doesn't work in higher category}

\todo{Do adjoint stuff here. maybe recast above in terms of adjoints.}
...
\begin{proof}
	...
	\todo{define these isoclasses and cospan notation}
	...
	\todo{I don't see the pentagram/triangle in Kenny's paper}   
\end{proof}
...
\todo{vert 1-maps can be isos instead of spans, but use spans for consistent}
...
\todo{why does hor comp preserve monics?}
\begin{proof}
	Let $\alpha, \alpha^\prime, \beta, \beta^\prime$ be composable 2-morphisms given by
	\[
	\begin{tikzpicture}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Revisit examples} % REVISIT EXAMPLES

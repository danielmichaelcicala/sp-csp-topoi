...
\end{equation}
We call $A, B, C$ and $D$ objects or \emph{0-morphisms}, $f$ and $g$ \emph{vertical morphisms}, $M$ and $N$ \emph{horizontal morphisms} and $a$ a \emph{$2$-morphism}. Note that a \emph{vertical morphism} is a morphism between \emph{0-morphisms} and a \emph{$2$-morphism} is a morphism between \emph{horizontal morphisms}. For the following definitions, we follow the notation of Shulman \cite{Shul}.
...
\begin{defn}
	...
	To match this definition with the more intuitive terms used, we say \emph{vertical morphisms} for the $\dblcat{D}_{0}$-morphisms, \emph{horizontal morphisms} for the $\dblcat{D}_{1}$-objects, and \emph{$2$-morphisms} for the $\dblcat{D}_{1}$-morphisms. As for notation, vertical and horizontal morphisms are written with the arrows $\to$ and $\hto$, respectively, and $2$-morphisms are drawn as in \eqref{diag:DblCatSquare}
\end{defn}
...
To bypass checking that a bicategory is monoidal, we will instead need to check that a double category is monoidal. To define a monoidal double category, however, we need the notion of a \emph{globular} $2$-morphism.  This is $2$-morphism whose source and target vertical morphisms are identities.
...
\begin{defn}
	...
	\begin{enumerate}
		...
		\]
		expressing that $\odot$ and $U$ are monoidal. Moreover, the following diagrams commute:
		\[
		\begin{tikzpicture}
...
Before we give the theorem that associates a symmetric monoidal bicategory to a suitable symmetric monoidal double category, we are first interested in how to get an ordinary bicategory from a double category. Indeed, for a double category $\dblcat{D}$ the \emph{horizontal edge bicategory} $H(\dblcat{D})$ of $\dblcat{D}$ is the bicategory whose objects are those of $\dblcat{D}$, morphisms are horizontal $\dblcat{D}$-morphisms, and $2$-morphisms are the globular $2$-morphisms.
...
\begin{proof}
	We will prove this for $\bicspmap{D}$ and use duality to obtain the result for $\bispmap{C}$. To begin, we will show that objects are self dual. Take an object $X$.  Define the evaluation morphism $e \from X + X \tocospan 0$ and coevaluation morphism $c \from 0 \tocospan X+X$ the following cospans:
	\[
...
\begin{proof}
	...
	Finally, there are a number of coherence axioms that must hold, but these are rather straightforward to check, though tedious.  
\end{proof}
...
\todo{make it more zippy and adjust for groupoid stuff}
...
As pullbacks preserve fibrations \cite{Brown,Heath}, it then follows that $\cat{\widehat{Gpd}}$ has pullbacks, where $\cat{\widehat{Gpd}}$ is the category of groupoids and fibrations. It then follows by Theorem \ref{thm:SpansSpansAreSMBicat} that $\cat{Sp(Sp(\widehat{Gpd})}$ which consists of
\begin{enumerate}
...
\end{enumerate}
is a symmetric monoidal bicategory which we conjecture is biequivalent to $\cat{Sp(Sp(Gpd))}$.
First, some preliminary definitions.
%\begin{defn}
%Let $U \colon \cat{C} \to \cat{D}$ be a functor. A morphism $f \colon c_{1} \to c_{2}$ in the category $\cat{C}$ is \textbf{cartesian (with respect to the functor U)} if for any object $c^\prime$ in $\cat{C}$ and morphism $g \colon c^\prime \to c_{2}$ and every $u \colon U(c^\prime) \to U(c_{1})$ such that $U(g)=U(f)u$, there exists a unique $h \colon c^\prime \to c_{1}$ such that $g=fh$ and $u=U(h)$.
%\[
%\begin{tikzpicture}[scale=1.5]
...
%\end{defn}
%\begin{defn}
%A functor $U \colon \cat{C} \to \cat{D}$ is a \textbf{fibration} if for any object $c$ in $\cat{C}$ and morphism $f \colon d \to F(c)$ there exists a cartesian morphism $\phi \colon c^\prime \to c$ such that $U(\phi)=f$.
%\end{defn}
\begin{defn}
...
\begin{defn}
	...
	\]
\end{defn}
The goal of this section is to prove that $\cat{Sp(Sp(\widehat{Gpd}))}$ is a symmetric monoidal bicategory. Let $\cat{Gpd}$ be the category of groupoids and functors and let $\widehat{\cat{Gpd}}$ be the category of groupoids and isofibrations. Before, we were using $\cat{Gpd}$ to denote the 2-category of groupoids, functors and transformations but the whole point of this approach is that we can deal only with the category $\cat{Gpd}$ and not the aforementioned 2-category and hence strict pullbacks instead of weak pullbacks. First we establish that an ordinary functor between groupoids can be replaced with an isofibration, modulo a natural isomorphism, and then that pullbacks in $\cat{Gpd}$ perserve isofibrations from which it will follow that $\widehat{\cat{Gpd}}$ has pullbacks. By Theorem \ref{thm:SpansSpansAreSMBicat} it will then follow that $\cat{Sp(Sp(\widehat{Gpd}))}$ is a symmetric monoidal bicategory.
\begin{thm}
...
\begin{thm}
	Let $F_{1} \colon G_{1} \to G$ and $F_{2} \colon G_{2} \to G$ be functors between groupoids. Then pullbacks in $\cat{Gpd}$ preserve isofibrations. Namely, if $Q$ is the strict pullback and $F_{2}$ is an isofibration, then $p$ is also an isofibration.
	\[
	\begin{tikzpicture}[scale=1.5]
	\node (A) at (0,0) {$Q$};
	\node (B) at (0,-1.5) {$G_{1}$};
...
\end{proof}
\begin{cor}
	$\cat{\widehat{Gpd}}$ has pullbacks.
\end{cor}
\begin{proof}
	Apply the above corollary twice to a pullback in $\cat{Gpd}$.
\end{proof}
\begin{thm}
	$\cat{Sp(Sp(\widehat{Gpd}))}$ is a symmetric monoidal bicategory.
\end{thm}
\begin{proof}
	$\cat{\widehat{Gpd}}$ has pullbacks by the above corollary and so this follows immediately by Theorem \ref{thm:SpansSpansAreSMBicat}.
\end{proof}
...
\todo{describe this picture and break it up into composition and don't write that equals sign}
\[
\begin{tikzpicture}[scale=0.5]
...
\begin{thm}
	The homotopy category $\text{ho}(\cat{F})$ is equivalent to the category $2\cat{Cob}$. \todo{include proof and don't use the term homotopy category}
\end{thm}

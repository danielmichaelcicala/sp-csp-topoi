...
\begin{abstract}
If $\cat{C}$ is a category with chosen pullbacks and a terminal object then, using a result of Shulman, we obtain a fully dualizable and symmetric monoidal bicategory $\cat{Sp(Sp(C))}$ whose objects are those of $\cat{C}$, morphisms are spans in $\cat{C}$, and 2-morphisms are isomorphism classes of spans of spans of $\cat{C}$. If $\cat{C}$ is a topos, the first author has previously constructed a bicategory $\cat{MonicSp(Csp(C))}$ whose objects are those of $\cat{C}$, morphisms are cospans in $\cat{C}$, and 2-morphisms are isomorphism classes of spans of cospans in $\cat{C}$ with monomorphic legs. This bicategory is also symmetric monoidal and even compact closed. Applications of such bicategories can be seen in graph rewriting rules as well as in Morton and Vicary's combinatorial approach to Khovanov's categorified Heisenberg algebra.
\end{abstract}
...
If $\cat{D}$ is a category with finite colimits, we can construct various bicategories using cospans in $\cat{D}$. This idea goes back to B\'enabou \cite{Be} in what was one of the first examples of a bicategory. Others, not exclusive to Courser, Rebro, and Stay \cite{Cour,Reb,Stay} have since proven related results. In this paper, we investigate properties of three types of cospan bicategories whose differences lie in the $2$-morphisms.  The $2$-morphisms considered are maps of cospans, cospans of cospans, and monic spans of cospans.  See Figure \ref{fig:2cells} for the shapes of these $2$-morphisms.
...
One of the first examples of a bicategory $\bispmap{D}$, given by B\'enabou \cite{Be} in his landmark paper, consisted of $\cat{D}$-objects, $\cat{C}$-cospans as 1-morphisms, and maps of $\cat{D}$-cospans as $2$-morphisms. The second listed author showed that $\bicspmap{D}$ is symmetric monoidal \cite{Cour}. However, a 2-morphism in $\bicspmap{D}$ is really just a special case of a cospan of cospans where one of the legs is an identity. Hence, we generalize $\bicspmap{D}$ to obtain a bicategory $\bicspcsp{D}$ with
\begin{enumerate}
	\item{objects of $\cat{D}$ as objects,}
	\item{cospans in $\cat{D}$ as morphisms, and}
	\item{isomorphism classes of cospans of cospans in $\cat{D}$ as $2$-morphisms.}
\end{enumerate}

In related work, the first listed author combined the ideas of using spans or cospans as 1-morphisms and 2-morphism to construct a bicategory $\bimonspcsp{T}$ that has cospans as morphisms and appropriate isomorphism classes of spans as $2$-morphisms \cite{Cic}. Namely, if $\cat{T}$ is a topos, there exists a bicategory consisting of objects of $\cat{T}$, morphisms as cospans of $\cat{T}$, and $2$-morphisms as isomorphism classes of spans of cospans where the morphisms of the inner span are required to be monomorphisms as depicted in Figure \ref{fig:2cells}. One might wonder whether the condition of the inner span being monic is necessary and if a more aesthetic result is possible, but it was shown that this monic condition on the spans is indeed necessary. 

We show in this paper that $\bicspcsp{D}$ and $\bimonspcsp{T}$ are symmetric monoidal categories. Though, instead of presenting the definition of a symmetric monoidal bicategory, we will simply say that it is very long.  Naturally, checking every axiom is time consuming. However, Shulman \cite{Shul} provides us a way out.  The idea is construct an \emph{isofibrant} double category that restricts, in a suitable sense, to the bicategory we are interested in.  If this double category is symmetric monoidal, then so will the `restricted' bicategory.  The advantage comes in checking that a double category is symmetric monoidal.  This is much more simple than checking the bicategory.

There is more nice structure to these bicategories that we investigate.  Indeed, it may not even be surprising that $\bicspmap{D}$, $\bicspcsp{D}$ and $\bimonspcsp{T}$ are all compact closed bicategories given the undirected nature of cospans. We prove they are.  Of course, compact closedness of bicategories is more involved than the 1-category case.  Stay \cite{Stay} has defined a compact closed bicategory to be one in which every object has a dual pair. Moreover, this dual pair must satisfy some coherence properties.  There is a stronger condition than compact closedness called full duality. Roughly, this means that every 1-morphism is both a right and left adjoint.  We are able to show that $\bicspcsp{D}$ is not only compact closed, but it is fully dual.  The other bicategories fail to be fully dual.

Observe that, even though we have been only speaking of bicategories whose 1-morphisms are cospans, we can obtain the bicategories by starting with a category $\cat{C}$ with finite limits then dualizing both 1-morphisms and 2-morphisms.  This gives bicategories $\bispmap{C}$ and $\bispsp{C}$. Or course, these will have the same structure as their counterparts $\bicspmap{D}$ and $\bicspcsp{D}$.

The reason we investigate these particular bicategories and their structure is to be able to use them in several applications. First we discuss an application for $\bispsp{C}$. Morton and Vicary \cite{MortVic} have conjectured the existence of a symmetric monoidal bicategory with
\begin{enumerate}
...
\end{tikzpicture}
\]
whose node labels indicate the graph morphism behavior. Here, the nodes $a$ and $b$ are inputs and $c$ is an output. The use of the terms `inputs' and `outputs' is justified by the composition, which can be thought of as connecting a graph's inputs to another's outputs. This connection can be performed when the inputs and outputs coincide and the composite 1-morphism is obtained using pushout.  For instance, we compose
\[
\begin{tikzpicture}[scale=1]
\draw[rounded corners] (-0.8,1.6) rectangle (1,3.4);
\draw[rounded corners] (1.8,3.6) rectangle (3.6,5.4);
...
\end{tikzpicture}
\]
Each $2$-morphism of $\cat{Rewrite}$ is a rewriting of one graph to another so that inputs and outputs are preserved. For instance
\begin{equation}
...
\begin{tikzpicture}[baseline=(current bounding box.center)]
	...
	\node[circle,draw,inner sep=4.5pt, fill] (b1) at (2.75,5) {};
	\node[circle,draw,inner sep=4.5pt, fill] (c1) at (2,4) {};
	\node[circle,draw,inner sep=4.5pt, fill] (d1) at (2.75,4) {};
...
The structure of the paper is as follows.  In Section \ref{sec:Span cospan bicats}, we introduce the bicategories $\bicspmap{D}$, $\bicspcsp{D}$, and $\bimonspcsp{T}$.  Then in Section \ref{sec:DoubleCategories}, we review how to use double categories to show that bicategories have symmetric monoidal structure.  Additionally, we discuss various notions of duality in bicategories.  Nothing in that section is new, but we will use those results in Sections \ref{sec:SpansMaps}, \ref{sec:SpansSpans}, and \ref{sec:SpansCospans} to show that our bicategories are symmetric monoidal and compact closed, as well as fully dualizable in the case of $\bicspcsp{D}$.  Finally, in Section \ref{sec:Applications}, we discuss two applications for bicategories of spans and cospans.  The first application is constructing a bicategory of spans of spans of groupoids that should prove useful in the study of quantum computing.  The second application uses $\bimonspcsp{Graph}$ to categorify the usual syntax for a category generated by a commutative monoid..
...
Our first task is to introduce three types of span and cospan bicategories.  While this section contains nothing new, it does allow us to recall important concepts and allows us to set our notation.  Throughout this paper, $\cat{C}$ will be a category with finite limits with chosen binary products and pullbacks and $\cat{D}$ will be a category with finite colimits with chosen binary coproducts and pushouts.  The full strength of finite limits or colimits are not always needed though it certainly simplifies our discussion.
...
Another way of promoting (co)span categories to bicategories is to consider different $2$-morphisms.  Rebro \cite{Reb} constructed a bicategory $\bispsp{C}$ with $\cat{C}$-objects, $\cat{C}$-spans $y \from x \tocospan z$, and isomorphism classes of $\cat{C}$-spans of spans, which are diagrams of the form
\[
\begin{tikzpicture}[]
	\node(A) at (0,0) {$x$};
	\node (B) at (1,1) {$y$};
...
\end{tikzpicture}
\]
Actually, the above diagram is an isomorphism class of spans of cospans where a pair of these are identified if there is a $\cat{C}$-isomorphism $\theta$ such that the following diagram commutes.
\[
\begin{tikzpicture}[scale=1]
	\node (X) at (-2,0) {$x$};
	\node (L) at (1.25,1.5) {$y$};
...
When studying intercategories, Grandis and Par\'{e} \cite{GranPare_Intercats} looked at spans of cospans and found a lax interchange law. Later, the first listed author showed that the interchange law is invertible \cite{Cic} when restricting attention to a topos $\cat{T}$ and monic span legs. This gives a bicategory $\bimonspcsp{T}$ with $\cat{T}$-objects, $\cat{T}$-cospans $y \from x \tocospan z$, and isomorphism classes of monic $\cat{T}$-spans of cospans. These $2$-morphisms are commuting diagrams
\[
\begin{tikzpicture}[]
	\node (A) at (0,0) {$x$};
	\node (B) at (1,1) {$y$};
...
Bicategories are nice, but symmetric monoidal bicategories are much nicer. Unfortunately, checking the axioms for a symmetric monoidal bicategory is daunting due to the sheer quantity of them.  However, using a result of Shulman \cite{Shul}, we can circumvent checking the axioms by, instead, promoting our bicategories to double categories and show that these double categories are symmetric monoidal.  This involves showing that a pair of categories are symmetric monoidal, a much more manageable task.  Better than symmetric monoidal bicategories are those that are compact closed. After first giving  a summary of Shulman's work, we discuss various notions of duality in bicategories as given by Lurie \cite{Lurie} and Stay \cite{Stay}.  These results will later be used to show that the various bicategories that we are interested in actually have the intended structure.  
...
\end{equation}
We call $A, B, C$ and $D$ \textbf{objects} or \textbf{0-cells}, $f$ and $g$ \textbf{vertical 1-morphisms}, $M$ and $N$ \textbf{horizontal 1-morphisms}, and $a$ a \textbf{2-morphism}. Note that a vertical 1-morphism is a morphism between objects and a $2$-morphism is a morphism between horizontal 1-morphisms. We follow the notation of Shulman \cite{Shul} with the following definitions, using bold font `$\dblcat{D}$' as a stand-alone letter or as the first letter in a label or name to denote a \emph{double category}.
...
\begin{defn}
	...
	To match this definition with the more intuitive terms used, we say \textbf{vertical 1-morphisms} for the $\dblcat{D}_{0}$-morphisms, \textbf{horizontal 1-morphisms} for the $\dblcat{D}_{1}$-objects, and \textbf{2-morphisms} for the $\dblcat{D}_{1}$-morphisms. As for notation, vertical and horizontal morphisms are written with the arrows $\to$ and $\hto$, respectively, and $2$-morphisms are drawn as in \eqref{diag:DblCatSquare}.
\end{defn}
...
To bypass checking that a bicategory is monoidal, we will instead need to check that a double category is monoidal. To define a monoidal double category, however, we need the notion of a \textbf{globular 2-morphism}.  This is a $2$-morphism whose source and target vertical $1$-morphisms are identities.
...
\begin{defn}
	...
	\begin{enumerate}
		...
		\[
		\begin{tikzpicture}
		\node (UL) at (0,1.5) {\footnotesize{$(M\otimes N) \odot U_{C\otimes D}$}};
		\node (LL) at (0,0) {\footnotesize{$M\otimes N$}};
		\node (UR) at (3.5,1.5) {\footnotesize{$(M\otimes N)\odot (U_C\otimes U_D)$}};
		\node (LR) at (3.5,0) {\footnotesize{$(M\odot U_C) \otimes (N\odot U_D)$}};
		%
		\path[->,font=\scriptsize]
...
\begin{defn}
	\label{def:Fibrant}
	We say that a double category is \textbf{fibrant} if every vertical 1-morphism has both a companion and a conjoint. If every \emph{invertible} vertical 1-morphism has both a companion and a conjoint, then we say the double category is \textbf{isofibrant}.
\end{defn}

The final piece we need to present the main theorem of this section is the following.  Given a double category $\dblcat{D}$, the \textbf{horizontal edge bicategory} $H(\dblcat{D})$ of $\dblcat{D}$ is the bicategory whose objects are those of $\dblcat{D}$, morphisms are horizontal 1-cells of $\dblcat{D}$, and $2$-morphisms are the globular $2$-morphisms.
...
In this section, we introduce various notions of duality in order to define a notion of compact closed bicategories in the vein of Stay \cite{Stay}.  We present the stronger concept of full dualizability also. Throughout, we suppress the notation for a monoidal operation and instead write $LR$ for the monoidal product of objects $L$ and $R$ and $fg$ for the monoidal product of morphisms $f$ and $g$.  This will reserve the symbol `$\otimes$' for the horizontal composition functor in the definition of a bicategory.
...
\begin{defn}
	...
	\end{tikzpicture}
	\]
	called cusp isomorphisms.  If this data satisfies the swallowtail equations in the sense that the diagrams in Figure \ref{fig:Swallowtail} are identities, then we call the dual pair \textbf{coherent}.
\end{defn}
...
Recall that a symmetric monoidal category is called \textbf{compact closed} if every object is part of a dual pair. We can generalize this idea to bicategories by introducing $2$-morphisms and some coherence axioms. The following definition is due to Stay \cite{Stay}.
...
The difference between showing compact closededness in categories versus bicategories is quite large because of the swallowtail equations.  Looking at Figure \ref{fig:Swallowtail}, it is no surprise that these can be incredibly tedious to work with.  Fortunately, Pstragowski proved a wonderful strictification theorem that effectively circumvents the need to consider the swallowtail equations.  
...
\begin{defn}
	There is an \textbf{adjunction} between 1-morphisms $f \from X \to Y$ and $g \from Y \to X$ in a bicategory if there are $2$-morphisms $\eta \from 1_{X} \to gf$ and $\epsilon \from fg \to 1_Y$ such that the compositions
	\[
	\begin{tikzpicture}
		\node (X) at (0,0) {$X$};
		\node (Y) at (2.5,0) {$Y$};
	...
	An \textbf{equivalence} between objects $X$ and $Y$ in a bicategory consists of a pair of morphisms $f \from X \to Y$ and $g \from Y \to X$ and invertible $2$-morphisms $\eta \from fg \to \id_Y$ and $\epsilon \from \id_X \to gf$.  In the case that $\eta$ and $\epsilon$ are the unit and counit of an adjunction $f \dashv g$, we say there is an \textbf{adjoint equivalence}.
\end{defn}

Let us now recast the definitions of duality in terms of adjunctions.  The benefit of this is that it allows us to define a stronger condition than duals.

Recall that a monoidal category $\cat{M}$ can be thought of as a bicategory $\cat{BM}$ with a single object, $\cat{M}$-objects as $1$-morphisms, and the $\cat{M}$-morphisms as the $2$-morphisms.  The composition functor of $\cat{BM}$ is given by the monoidal structure of $\cat{M}$. Then, an $\cat{M}$-object $L$ has a right dual $R$ if and only if $L \dashv R$ in $\cat{BM}$. 

Given a bicategory $\cat{B}$, define its truncation $|| \cat{B} ||$ to be the category obtained by keeping the same objects and taking as morphisms the isomorphism classes of $1$-morphisms in $\cat{B}$. Now, suppose that $\cat{B}$ has a monoidal structure. The category $|| \cat{B} ||$ inherits this structure and so we can consider the bicategory $\cat{B} (|| \cat{B} ||)$. This allows us to define duals in $|| \cat{B} ||$ via adjoints as above. A $\cat{B}$-object $L$ has a right dual if the $(|| \cat{B} ||)$-object $L$ has a right dual.

Having defined dual pairs in terms of adjoints, we can give an even stronger condition called `fully dual'.  We will take a simplified approach and only say enough for our needs. For a more fundamental and general account of full dualizability see \cite{Lurie,Piotr}. 

To prepare for the definition of fully dual pairs, we construct the following universal bicategory. Let $\cat{S}$ be a symmetric monoidal bicategory. There exists another symmetric monoidal bicategory $\cat{S}^{\text{fd}}$ with duals and a symmetric monoidal functor $i \from \cat{S}^{\text{fd}} \to \cat{S}$.  The idea behind the construction of $\cat{S}^{\text{fd}}$ is iterating a process of removing morphisms from $\cat{S}$ that do not have both a left or right adjoint. Then $\cat{S}^{\text{fd}}$ has the universal property where, given any other symmetric monoidal bicategory $\cat{S}'$ with duals and a symmetric monoidal functor $F \from \cat{S}' \to \cat{S}$, then $F$ factors uniquely up to isomorphism through $i$. In fact, $\cat{S}^{\text{fd}}$ is the maximal sub-bicategory of $\cat{S}$ with respect to the property that every morphism has left and right adjoints.  
...
On the surface, it seems a bit dire to determine whether an object is fully dualizable, but there is a nice characterization of this property that makes checking for it a bit easier.
...
In this section, we show that the bicategory $\bicspmap{D}$ of $\cat{D}$-objects, $\cat{D}$-cospans, and $\cat{D}$-maps of cospans is symmetric monoidal and compact closed. By duality, all results will also hold for $\bispmap{C}$. To this end, we construct a double category $\dblcspmap{D}$ whose horizontal edge bicategory is $\bicspmap{D}$.  The objects of $\dblcspmap{D}$ are the $\cat{D}$-objects, vertical 1-morphisms are the $\cat{D}$-morphisms, horizontal 1-morphisms are $\cat{D}$-cospans, and $2$-morphisms are triples of $\cat{D}$-morphisms between two cospans in $\bold{D}$ so that the diagram
\[
\begin{tikzpicture}
...
\end{tikzpicture}
\]
commutes.  This double category was introduced by Dawson, Par\'{e}, and Pronk \cite{DawsonParePronk} and previously discussed by one of the authors \cite{Cour}.
...
\begin{proof}
	Let $f \from a \to b$ be a vertical 1-morphism in $\dblcspmap{D}$.  The companion $\widehat{f}$ is the cospan 
	\[
...
\begin{proof}
	To begin, we will show that objects are self dual. Take an object $X$.  Define the evaluation morphism $e \from X + X \tocospan 0$ and coevaluation morphism $c \from 0 \tocospan X+X$ to be the following cospans:
	\[
...
In this section, we will show that the bicategories $\bispsp{C}$ and $\bicspcsp{D}$ are symmetric monoidal. Again, we will only prove this for $\bicspcsp{D}$.  

\begin{defn}
\label{def:DblCatSpanSpan}
	Define a double category $\bicspcsp{D}$ whose objects are the $\cat{D}$-objects, vertical morphisms are given by isomorphism classes of $\cat{D}$-cospans, horizontal morphisms are given by $\cat{D}$-cospans, and $2$-morphisms are cospans of cospans in $\cat{D}$ making the evident diagram commute (see Figure \ref{fig:2cells}). 
\end{defn}

% DOUBLE CATEGORIES
\begin{lem}
	\label{lem:SpanSpanDoubleCat}
	The data for $\dblcspcsp{D}$ defines a double categories.  
\end{lem}

\begin{proof}
	To simplify notation, denote $\dblcspcsp{D}$ by $\dblcat{D}$. The object category $\dblcat{D}_0$ is the usual category $\cat{Csp(D)}$ of isomorphism classes of cospans in $\bold{D}$.  The arrow category $\dblcat{D}_1$ has as objects as $\cat{D}$-cospans and morphisms as isomorphism classes of cospans of cospans. A morphism from $b \from a \tocospan c$ to $b'' \from a'' \tocospan c''$ is given by a diagram
	\begin{equation}
...
\begin{proof}
	...
	Finally, we check that the coherence axioms, namely (a)-(k) of Definition \ref{def:MonoidalDoubleCategory}, hold. These are rather straightforward, if tedious, to verify. For example, if we have
\[
		\begin{tikzpicture}
			\node (a) at (0,0) {$a$};
			\node (b) at (1,.5) {$b$};
		...
		\end{tikzpicture}
	\]
then following diagram \eqref{diag:MonDblCat} around the top right gives the following sequence of cospans
\[
		\begin{tikzpicture}
			\node (a) at (-3,0) {$a+t$};
...
\node (M1) at (1,1.5) {$(M_1 \odot (M_2 \odot M_3)) \otimes (N_1 \odot (N_2 \odot N_3)) =$};
		...
		\end{tikzpicture}
	\]
Following diagram \eqref{diag:MonDblCat} around the bottom left gives the following sequence of cospans. 
\[
		\begin{tikzpicture}
			\node (a) at (-3,0) {$a+t$};
			\node (b) at (1,.5) {$((b+w)+_{c+v}(d+w))+_{e+x}(f+y)$};
...
\node (M1) at (1,1.5) {$(M_1 \odot (M_2 \odot M_3)) \otimes (N_1 \odot (N_2 \odot N_3)) =$};
		...
		\end{tikzpicture}
	\]
In other words, we have that the diagram
\[
		\begin{tikzpicture}
			\node (a) at (-4,0) {$a+t$};
		...
		\end{tikzpicture}
	\]
 commutes.  The vertical 1-morphisms on the left and right are identities. The vertical 1-morphisms in the center and the horizontal morphisms are the universal maps.  The top cospan is the same as the bottom cospan, making a bracelet-like figure in which all faces commute.  The other diagrams witnessing coherence are similarly showed.
\end{proof}
...
\begin{proof}
	If we have a vertical 1-morphism $b: a \tocospan c$ given by an isomorphism class of cospans, then a companion of $b$ is the cospan $\widehat{b} = b$. Consider $2$-morphisms
	\[
	\begin{tikzpicture}
		\node (A) at (1,1) {$a$};
		\node (B) at (2,1) {$b$};
	...
	\end{tikzpicture}
	\]
	whose arrows are either from the given $1$-morphism or identities.  These make the diagrams required to be a companion commute. A conjoint of $b$ is given by $\check{b} \from c \tocospan a$. This is just the companion of $b$ written in reverse order. It follows that $\dblcspcsp{D}$ is fibrant. In particular, it is isofibrant.
\end{proof}
...
\begin{thm}
	...
	Let $\bold{D}$ be a category with chosen pushouts and an initial object. Then $\bicspcsp{D}$ is symmetric monoidal bicategories.
\end{thm}
...
\begin{thm}
	...
	Every object $\bicspcsp{D}$ is fully dualizable.
\end{thm}

\begin{proof}
	We first show that objects are self dual.  Take an object $X$. Define the evaluation morphism $e \from X+X \to 0$ and coevaluation morphism $c \from 0 \to X+X$ as follows:
	\[
		e = (X+X \xto{\nabla} X \xleftarrow{!} 0)
		\quad \quad
...
\begin{proof}
	Denote $\dblmonspcsp{T}$ by $\dblcat{M}$.  The object category $\dblcat{M}_0$ is $\cat{Span(T)}$ and the arrow category $\dblcat{M}_1$ is made from $\cat{T}$-cospans and isomorphism classes of $\cat{T}$-spans with monic legs between the apexes of the $\cat{T}$-cospans.  
...
\begin{proof}
	...
	Because $\dblmonspcsp{T}$ has the same horizontal 1-cells as $\dblcspcsp{T}$, we can use the same globular isomorphisms for each case but turning vertically aligned spans into cospans by using inverses.  It follows that the required diagrams will commute.
\end{proof}
...
\item{Equivalence classes of spans of spans of groupouds as 2-morphisms, where the diagram commutes up to natural isomorphism.}
\end{enumerate}
...
Let us now consider the category $|| \cat{C} ||$ obtained through decategorifying $\cat{C}$ by taking the $1$-morphisms up to homotopy equivalence.  This induces the subcategory $|| R(\cat{M}) ||$ with objects the same as $R (\cat{M})$ and morphisms generated by $R(\mu)$ and $R(\eta)$ up to homotopy equivalence.

\begin{thm}
	The category $||R(\cat{M})||$ is generated by a commutative monoid.
\end{thm}
